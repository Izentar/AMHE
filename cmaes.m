function BestCost = cmaes(initRandomMult)
...
CostFunction=@Rosenbrock;   % Cost Function

nVar=10;                % Number of Unknown (Decision) Variables

VarSize=[1 nVar];       % Decision Variables Matrix Size

VarMin=-2;             % Lower Bound of Decision Variables
...
%ps{1}=rand(VarSize) .* initRandomMult;
ps{1}=normrnd(0,initRandomMult,VarSize);
pc{1}=zeros(VarSize);
C{1}=eye(nVar);
sigma{1}=sigma0;
